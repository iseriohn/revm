...
pub fn prepare_state(cache_db: &mut CacheDB<EmptyDB>) -> Result<()>{
    ...
    let mut ethersdb = EthersDB::new(Arc::clone(&client), Some(13052291)).unwrap();
...
pub fn init_evm() -> Result<EVM<CacheDB<EmptyDB>>> {
    // initialise empty in-memory-db
    let mut cache_db = CacheDB::new(EmptyDB::default());

    prepare_state(&mut cache_db).unwrap();
